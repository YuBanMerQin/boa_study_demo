...
// request中装载数据的仍然有空余,读取管道中的数据
	...
	// 因为每次读取出来的数据都加了'\0'，所以输出的时候，数据会输出不完整。
	*(req->header_end + bytes_read) = '\0';

	if (bytes_read == 0)	// PIPE中读取不出字节，不是出错，已经读取到文件的结束位置
	{      /* eof, write rest of buffer */
		...
		req->cgi_status = CGI_DONE;
		printf("113 pipe: req->cgi_status = CGI_DONE\n");
...
//回复HTTP请求头和数据之间的换行。
		...
		req->cgi_status = CGI_DONE;
		printf("135 pipe: req->cgi_status = CGI_DONE\n");

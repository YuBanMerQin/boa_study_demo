...
/* Pointed out 3-Oct-1999 by Paul Saab <paul@mu.org> */
	...
	printf("new print: ConnInfo: header_line:{%s}, local_ip_addr:{%s}\n", conn->header_line, conn->local_ip_addr);
...
void process_requests(int server_sock)
{
	...
	while (current)
	{
		...
		if (sigterm_flag) SQUASH_KA(current);
		 ...
		 */
		if (pending_requests) get_request(server_sock);

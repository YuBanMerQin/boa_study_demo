...
int process_cgi_header(request* req)
{
	...
	{
		printf("req->http_version == HTTP09\n");
		if (*(c + 1) == '\r')
			req->header_line = c + 2;
		else
			req->header_line = c + 1;
	...
	{ /* got a location header */
		...
		{   /* virtual path */
			log_error_doc(req);
			fprintf(stderr, "server does not support internal redirection: " "\"%s\"\n", buf + 10);
	...
	{
		// 如果是GET请求，则会直接进入这个段，并且该函数只会执行一次
		...
		if (req->method == M_HEAD)
		{
			...
			printf("165 cgi-header: req->cgi_status = CGI_DONE\n");
		}
		...
		memmove(dest, req->header_line, howmuch);		// 将HTTP消息体移动到HTTP头BUF的下面
		req->buffer_end += howmuch;
		req->header_line = req->buffer + req->buffer_end;	// 更改HTTP消息体存放的位置为HTTP头BUF中
		req->header_end = req->header_line;
		req_flush(req);
						// client_stream	header_line指向的是这里,所以不需要释放
		if (req->method == M_HEAD)
			return 0;
	}
	return 1;

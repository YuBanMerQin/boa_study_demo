...
int req_write(request* req, const char* msg)
{
	...
	{
		log_error_doc(req);
		fprintf(stderr, "There is not enough room in the buffer to copy %u bytes (%d available). "
						"Shutting down connection.\n", msg_len, BUFFER_SIZE - req->buffer_end);
...
int req_write_escape_html(request* req, const char* msg)
{
	...
	{
		log_error_doc(req);
		fprintf(stderr, "Ran out of Buffer space (%d chars left)!  [req_write_escape_html]\n", left);
		req->status = DEAD;
		return -1;
	}
	return req->buffer_end;
 ...
 */
// 如果有阻塞的request请求转移到ready队列中,刷新这些buf中的数据,并将他们发送出去
int req_flush(request* req)
{
	unsigned bytes_to_write = req->buffer_end - req->buffer_start;
	if (req->status > DONE)
		return -2;//刷新的时候刚好超时

	if (bytes_to_write)
	{
		int bytes_written = write(req->fd, req->buffer + req->buffer_start, bytes_to_write);

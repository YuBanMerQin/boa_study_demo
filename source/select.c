...
void loop(int server_s)
{
	...
	while (1)
	{
		...
		if (max_fd)
		{
			...
			if (select(max_fd + 1, BOA_READ, BOA_WRITE, NULL, (request_ready || request_block ? &req_timeout : NULL)) ==
				-1)
			{
...
static void fdset_update(void)
{
	...
	for (current = request_block; current; current = next)
	{
		...
		else
		{
			...
			case WRITE:
			case PIPE_WRITE:
				if (FD_ISSET(current->fd, BOA_WRITE)) ready_request(current);
				else
				BOA_FD_SET(current, current->fd, BOA_WRITE);
				break;
			case BODY_WRITE:
				if (FD_ISSET(current->post_data_fd, BOA_WRITE)) ready_request(current);
				else
				{
				...
				break;
			case PIPE_READ:
				if (FD_ISSET(current->data_fd, BOA_READ)) ready_request(current);
				else
				{
			...
			case DEAD:
				...
				break;
			default:
				if (FD_ISSET(current->fd, BOA_READ)) ready_request(current);
				else

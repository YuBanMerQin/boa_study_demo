...
#define __TEST__
#ifdef __TEST__
    printf(" server_name:{%s}, single_post_limit:{%d}, vhost_root:{%s}, virtualhost:{%d}, vhost_root:{%s}, document_root:{%s}",
           server_name, single_post_limit, vhost_root, virtualhost, vhost_root, document_root);
#endif
    ...
    if (ka_timeout < 0)
        ka_timeout = 0; /* not worth a message */
    /* save some time */

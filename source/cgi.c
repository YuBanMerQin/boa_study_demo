 ...
 */
// 创建key=value， 参数extra代表空间的前面需要预留多大的空间
...
int add_cgi_env(request* req, const char* key, const char* value, int http_prefix)
{
	...
	if (req->cgi_env_index < CGI_ENV_MAX)
	{
		...
		{
			log_error_doc(req);
			fprintf(stderr, "Unable to generate additional CGI environment variable -- ran out of memory!\n");
			return 0;
		}
		if (prefix_len)
			memcpy(p, "HTTP_", 5);
	...
	log_error_doc(req);
	fprintf(stderr, "Unable to generate additional CGI Environment variable \"%s%s=%s\" -- not enough space!\n",
...
static int complete_env(request* req)
{
	...
	log_error_doc(req);
	fprintf(stderr, "Not enough space in CGI environment for remainder of variables.\n");
	return 0;
}
...
static void create_argv(request* req, char** aargv)
{
	...
	fprintf(stderr, "create_argv function: query_string:%s pathname:%s\n", q, aargv[0]);
...
{
	printf("init_cgi function: query_string:%s pathname:%s, %d\n",
			req->query_string, req->pathname, req->cgi_type);
	int child_pid;
	int pipes[2];
	int use_pipes = 0;
	...
	{
		int i;
		for (i = 0; i < req->cgi_env_index; ++i) log_error_time();
	...
	if (req->cgi_type == CGI || (!req->cgi_type && (req->pathname[strlen(req->pathname) - 1] == '/')))
	{
		use_pipes = 1;
		if (pipe(pipes) == -1)
		{
	...
	case 0:
		...
		else
		{
			...
			{
				fprintf(stderr,"dirmaker: %s, pathname:{%s}, request_uri:{%s}\n", dirmaker, req->pathname, req->request_uri);
				execl(dirmaker, dirmaker, req->pathname, req->request_uri, (void*)NULL);
			}
#ifdef GUNZIP
			else
				execl(GUNZIP, GUNZIP, "--stdout", "--decompress", req->pathname, (void*)NULL);
#endif
		}
		/* execve failed */
		log_error_doc(req);
